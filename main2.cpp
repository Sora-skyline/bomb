...
//
//template <typename T> inline T lerp(const T& v0, const T& v1, float t) {//插值
...
//}
//
//float noise(const Vec3f& x) {//噪声函数
//    Vec3f p(floor(x.x), floor(x.y), floor(x.z)); //x0
//    Vec3f f(x.x - p.x, x.y - p.y, x.z - p.z);  // t = x - x0
//    f = f * (f * (Vec3f(3.f, 3.f, 3.f) - f * 2.f));// 插值函数 t = 3t^2 - 2t^3
...
//}
//
//float fractal_brownian_motion(const Vec3f& x) { // 分型布朗运动this is a bad noise function with lots of artifacts. TODO: find a better one
//    Vec3f p = rotate(x);                                           // 把噪声叠加
//    float f = 0;
